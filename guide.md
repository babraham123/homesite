...
- Install sass
```bash
SA_VERSION=$(curl -s "https://api.github.com/repos/sass/dart-sass/releases/latest" | grep -Po '"tag_name": "v\K[0-9.]+')
wget "https://github.com/sass/dart-sass/releases/download/v${SA_VERSION}/dart-sass-${SA_VERSION}-linux-x64.tar.gz" -O - | tar xz
mv sass /usr/bin
rm -rf src
```
